    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            ProgressView(value: progressValue())
                .progressViewStyle(
                    LinearProgressViewStyle(tint: Color("OnSurface"))
                )
                .background(Color("SurfaceContainer"))  // Background bar color
                .cornerRadius(2)
                .frame(width: .infinity, height: 4).animation(
                    .easeInOut(duration: 0.3), value: currentStep)
            Text(currentSection).font(
                Font.custom("Plus Jakarta Sans Medium", size: 14)
            )
...
#Preview {
    VStack {
        ProgressStepper(
            totalSteps: 5, currentStep: 5, currentSection: "About Me"
        )
        .padding()
    }
}
